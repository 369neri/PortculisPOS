...
main() {
  group('Keypad Cubit', () {
    ...
    blocTest(
      ...
      build: () => KeypadCubit(const KeypadState([])),
      act: (bloc) => bloc.add('1'),
    ...
    blocTest(
      ...
      expect: () => [
        const KeypadState([1,2,3,4,5,6,7,8,9]),
        const KeypadState([1,2,3,4,5,6,7,8,9,0])
      ],
    );
    ...
    blocTest(
      ...
      expect: () => [const KeypadState([1,0,0])],
    );
    ...
    blocTest(
      ...
      expect: () => [const KeypadState([1,0,0,0])],
    );

    blocTest(
      'should remove last integer from list on edit action', 
      build: () => KeypadCubit(const KeypadState([1,2])),
      act: (bloc) => bloc.edit(),
      expect: () => [const KeypadState([1])],
    );
    ...
    blocTest(
      ...
      build: () => KeypadCubit(const KeypadState([1,3,5,7])),
      act: (bloc) => bloc.clear(),
      expect: () => [const KeypadState([])],
    );

    blocTest('should store value and clear buffer when {times} command is entered', 
      build: () => KeypadCubit(const KeypadState([1,2,3])),
